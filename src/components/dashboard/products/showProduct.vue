...
const deleteImgProduct = async (img: ProductImagesInterface, type: string) => {
  let imgIndex = product.value.images.findIndex((item: ProductImagesInterface) => {
  ...
  try {
    ...
    if (response.success) {
      ...
      if (type !== 'banner') {
        ...
        product.value.banner.splice(imgIndex, 1)
      }
    }
  } catch (error) {
  }
